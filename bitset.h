...
typedef struct 
{
    ...
    // T: It is useful to allocate the entire struct CBitSet like a single piece of memory, even if packed_data has
    // a variable length.
    ...
    // Bitset *pointer = (CBitSet*)malloc(sizeof(CBitSet) + sizeof(WORD_TYPE) * size));
    // In this way packed_data start directly from the address next to capacity.
    // T: How we already sayed, the advantage to use this trick is that all the CBitSet is allocated in contigous memory.
